...
import { Download, RefreshCw, Search, Save, Trash2 } from "lucide-react";
...
const AdminTermine = () => {
  ...
  const fetchItems = useCallback(async () => {
    ...
    if (error) {
      ...
      setItems(data as Termin[]);
      setNoteDrafts(
        Object.fromEntries((data as Termin[]).map((l) => [l.id, l.notes ?? ""]))
      );
    }
    setLoading(false);
  }, [toast]);
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `termine_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    <div className="mx-auto px-6 py-8" style={{ maxWidth: 1280 }}>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          >
            Beratungstermin-Anfragen von der Landingpage.
          </p>
        </div>
